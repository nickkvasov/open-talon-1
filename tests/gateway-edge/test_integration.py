...
Run:
    pytest tests/gateway-edge/ -m integration -v
...
To also tear down infra after the session, pass --gateway-teardown-infra:
    pytest tests/gateway-edge/ -m integration --gateway-teardown-infra
"""
...
_GW_DIR       = _REPO_ROOT / "services" / "gateway-edge"
_GW_PYTHONS   = (
    _REPO_ROOT / ".venv" / "bin" / "python",
    _REPO_ROOT / "services" / "gateway-edge" / ".venv" / "bin" / "python",
    _REPO_ROOT / "api-gateway" / ".venv" / "bin" / "python",
)
...
def gateway_process(infrastructure):
    ...
    Uses the repository virtualenv interpreter when available.
    """
    ...
    proc = subprocess.Popen(
        [python, "-m", "uvicorn", "gateway_edge.main:app",
    ...
    try:
        ...
        if stdout:
            print(f"[integration] Gateway stdout:\n{stdout}")
        if stderr:
            print(f"[integration] Gateway stderr:\n{stderr}")
        raise
...
def _select_gateway_python() -> str:
    ...
    raise RuntimeError(
        "No usable Python interpreter found for gateway integration tests; "
