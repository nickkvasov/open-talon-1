...
from .telemetry import PayloadRedactionPolicy, redact_payload
...
ObservabilityProvider = RuntimeObservability
...
class LangfuseRuntimeObserver:
    ...
    def record_event(
        ...
        *,
        ...
        with self.start_span(name=name, input=input, metadata=metadata):
            return None
...
def build_observability_provider_from_env(
    *,
    service_name: str,
    legacy_env_prefix: str | None = None,
) -> ObservabilityProvider:
    provider_name = _first_env(
        "OPEN_TALON_OBSERVABILITY_PROVIDER",
        _legacy_env_name(legacy_env_prefix, "OBSERVABILITY_PROVIDER"),
    )
    ...
    if provider_name == "otlp":
        endpoint = _first_env(
            "OPEN_TALON_OTLP_HTTP_ENDPOINT",
            _legacy_env_name(legacy_env_prefix, "OTLP_HTTP_ENDPOINT"),
        ) or "http://127.0.0.1:4318/v1/traces"
        return OtlpHttpObservabilityProvider(
            endpoint=endpoint,
            service_name=(
                _first_env(
                    "OPEN_TALON_OTLP_SERVICE_NAME",
                    _legacy_env_name(legacy_env_prefix, "OTLP_SERVICE_NAME"),
                )
                or service_name
            ),
            environment=_first_env(
                "OPEN_TALON_OTLP_ENVIRONMENT",
                _legacy_env_name(legacy_env_prefix, "OTLP_ENVIRONMENT"),
            ),
            redaction_policy=PayloadRedactionPolicy(
                rich_payloads_enabled=_get_bool(
                    "OPEN_TALON_OBSERVABILITY_RICH_PAYLOADS",
                    True,
                    legacy_name=_legacy_env_name(
                        legacy_env_prefix,
                        "OBSERVABILITY_RICH_PAYLOADS",
                    ),
                )
            ),
        )
...
def _legacy_env_name(prefix: str | None, suffix: str) -> str | None:
    if not prefix:
        return None
...
def _get_bool(name: str, default: bool, *, legacy_name: str | None = None) -> bool:
    value = _first_env(name, legacy_name)
    if value is None:
        return default
