...
Unit tests (default — no docker needed):
    pytest test/api-gateway/ -m "not integration"

Integration tests (requires running infra + gateway):
    pytest test/api-gateway/ -m integration
...
# ── Make the gateway importable from test/api-gateway/ ───────────────────────
...
class MockEventService:
    ...
    async def wait_for_response(self, correlation_id: UUID, timeout=None):
        from app.models import KafkaChatResponse
...
def patched(monkeypatch, session_store, history_store, mock_event):
    ...
    monkeypatch.setattr("app.services.session.create_session", _create_session)
    monkeypatch.setattr("app.services.session.get_session",    _get_session)
    monkeypatch.setattr("app.services.session.touch_session",  _touch_session)
    monkeypatch.setattr("app.services.session.delete_session", _delete_session)
    monkeypatch.setattr("app.services.session.setup_valkey",   AsyncMock())
    monkeypatch.setattr("app.services.session.teardown_valkey", AsyncMock())
    ...
    monkeypatch.setattr("app.services.history.save_message",   _save_message)
    monkeypatch.setattr("app.services.history.get_history",    _get_history)
    monkeypatch.setattr("app.services.history.delete_history", _delete_history)

    # ── Postgres lifecycle ────────────────────────────────────────────────────
    monkeypatch.setattr("app.db.postgres.setup_postgres",   AsyncMock())
    monkeypatch.setattr("app.db.postgres.teardown_postgres", AsyncMock())

    # ── EventService singleton ────────────────────────────────────────────────
    monkeypatch.setattr("app.services.events.event_service",      mock_event)
    monkeypatch.setattr("app.routers.chat.event_svc.event_service", mock_event)
...
async def client(patched) -> AsyncIterator[AsyncClient]:
    """Async HTTPX client against the mocked ASGI app."""
